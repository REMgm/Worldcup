...
Live knockout bracket, win-chance signals, match momentum and player form for the 2026 World Cup. Read-only editorial: **signals, not tips**. Built to the v1 build spec — the tournament is at the knockout stage, so the bracket is the product.
...
Win chances come from the compounding intelligence loop in `lib/predictor.ts`: an Elo-style rating updated by every cached result (margin-weighted, shootout wins discounted), blended with the latest market-implied probabilities — each new result compounds into the next round's percentages. Hot takes ("The Signals") are generated by a deterministic rules engine (`lib/takesEngine.ts`) over form + odds deltas — market walk-backs, coin flips, xG overperformance, form spikes. No LLM in the hot path.
